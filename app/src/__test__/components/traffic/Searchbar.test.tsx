...
const testDataArriveTime = [
  {
    ...
    EstimateTime: '2021-12-31T01:00:54+08:00',
  },
  {
    Direction: 1,
    RouteID: '10148',
...
describe('components/traffic/Searchbar', () => {
  beforeEach(() => {
    cleanup();
    (getRoutesByCity as jest.Mock).mockResolvedValue(testDataRoute);
    (getStopsByCityAndRouteName as jest.Mock).mockResolvedValue(testDataStop);
    (getArrivalTimeByCityAndRouteName as jest.Mock).mockResolvedValue(
      testDataArriveTime
    );
  });
  ...
  test('initial', async () => {
    ...
    expect(store.getState().route.stops[0]).toStrictEqual({
      ...testDataStop[0].Stops[0],
      estimateTime: testDataArriveTime[0].EstimateTime,
      stopStatus: testDataArriveTime[0].StopStatus,
    });
  });
  ...
  test('Change city & route selector and direction button', async () => {
    ...
    expect(store.getState().route.stops[0]).toStrictEqual({
      ...testDataStop[1].Stops[0],
      estimateTime: testDataArriveTime[1].EstimateTime,
      stopStatus: testDataArriveTime[1].StopStatus,
    });
  });
});
